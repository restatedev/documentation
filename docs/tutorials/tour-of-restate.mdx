...
To interact with Restate, you first need to retrieve the `RestateContext`. This is the entrypoint of all communication with the runtime.  You can do this as follows:
```   typescript
const ctx = restate.useContext(this);
```
...
- Connection is established by runtime
...
The sleep that we implemented in the ticket service. 
