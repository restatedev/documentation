...
:::note To skip the implementation, follow along by looking at the code under `src/part1`.
:::
...
Restate makes their handlers suspend to free up the resources for other invocations.
...
### 📝 Try it out
The `UserSessionService/ExpireTicket` handler is called when a user did not proceed with the payment before the timeout.
...
:::info 🚩 Explore the intermediate solution in `src/part1`, and run it with `npm run part1`.
:::

## Durable timers
:::note To skip the implementation, follow along by looking at the code under `src/part2`.
:::

Restate suspends a handler when it's waiting on external input.
...
:::info 🚩 Explore the intermediate solution in `src/part2`, and run it with `npm run part2`.
:::

## Side effects

:::note To skip the implementation, follow along by looking at the code under `src/part3`.
:::
...
:::info 🚩 Explore the intermediate solution in `src/part3`, and run it with `npm run part3`.
:::
## Resiliency

:::note To skip the implementation, follow along by looking at the code under `src/part4`.
:::
...
As we said, the runtime automatically does retries of handler invocations.
However, if you do a side effect operation in your handler and the side effect itself failed,
    then this failure will be durably stored in the logs and it will not be retried.
