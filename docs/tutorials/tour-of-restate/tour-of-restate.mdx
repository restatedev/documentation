...
we can call the `UserSessionService/Checkout` handler to proceed with the purchase.
...
When the `addTicket` method is called, it first needs to reserve the ticket for the user. It does that by calling the `TicketService/Reserve` handler.
...
To interact with Restate, we need to retrieve the `RestateContext`. This is the entrypoint of all communication with the runtime. That is done in the first line of the highlighted code snippet.
To resolve this, make sure you have the following import:
```typescript
import * as restate from "@restatedev/restate-sdk";
```

Once we have access to the `RestateContext`, we call the ticket service.
We use the client that was generated by `proto-ts` and supply the RestateContext of the current invocation (`ctx`) to the client to make sure that it uses the SDK to send the requests.
Then we call the reserve method, which returns a Promise that gets resolved with the BoolValue response.

Let's see try it out. Run the services via `npm run app` and send a request to `UserSessionService/AddTicket`, as we did [previously](#running-the-services-and-runtime).
...
To also see which messages were sent between the service and the runtime use `RESTATE_DEBUG_LOG=MESSAGES`.
...
For `tickets` field the `CheckoutFlowRequest`, you can use a hardcoded array of strings for now: `["456"]`.
...
### 📝 Exercise
The `expireTicket` method in the user session service is called when a user did not proceed to payment before the end of the timeout.
Let the `expireTicket` method asynchronously call the `TicketService/Unreserve` handler.
Call the `expireTicket` method to check if your code works.
...
## Awakeables
Until now, all the calls were done between Restate services.
But what if you want to do include an external service in your workflow?

For example, you want a Restate service to put a message on Kafka.
This message gets ingested by a non-Restate service that processes it
    and then triggers the Restate service to resume processing.
This is what you can use awakeables for.
Awakeables are similar to the callback task token pattern:
1. Service A generates an ID.
2. Service A supplies that ID to an external service.
3. Once the external service has finished its task, it sends the ID back to Restate.
4. When Restate receives the ID, it invokes service A to resume processing.
...
Calling `ctx.awakeable` gives you an object with two entries. A String ID and a promise. The promise gets resolved with the payload that the external service attached to it.
You need to supply the data return type when you create the awakeable. In this case, the return payload will be a String.

Similar to synchronous and asynchronous communication, the method invocation is suspended while waiting on the return of the ID.


[//]: # (TODO include an example in the tutorial.)

Let's test this by sending over the id via a inbackground call to another method
and the other method then awakes the first method.

## Durable timers
:::info Bugs?
🚩 Explore the solution in `src/part1`, and run it with `npm run part1`.

Once you have your code up and running again, you can continue with the tutorial!
:::
...
In the service logs, you can see the `reserve` handler processing the sleep, then suspending, and then resuming again after the sleep completed.
...
Simply remove the `inBackground()` wrapper around the call, and end up with:
...
Now you see in the logs that both services get suspended. The user session service gets suspended because it waits for a response from the `reserve` handler
...
If you do a sleep operation, the invocation is ongoing.
    If you do a delayed call, the invocation is not ongoing until the delay has passed, so no key is blocked.
...
:::tip
Use the concurrency guarantees of keyed (and singleton services) to your
...
just like the database table behind it. Restate ensures that there are never concurrent writes
to the same row.
:::
...
The service is an unkeyed service that has a `Checkout` method that takes a `CheckoutFlowRequest` and returns a
...
Run `npm run proto` to generate the Typescript code.
...
Now that we implemented the checkout handler, we can also finalize the `UserSessionService/ExpireTicket`.
...
Let's track the status of the tickets in the ticket service by storing it in the state. Do the exercises
listed below and check your implementation by running the service and calling the methods.

1. Implement the `TicketService/Reserve` handler.
...
2. Implement the `TicketService/Unreserve` handler. The method should set the
...
3. Implement the `TicketService/MarkAsSold` handler. The method should set the
...
In the next section, we will implement the `CheckoutService/Checkout` handler
...
## Side effects
:::info Bugs?
🚩 Explore the solution in `src/part2`, and run it with `npm run part2`.

Once you have your code up and running again, you can continue with the tutorial!
:::

Earlier, we explained the replay mechanism of Restate.
...
create and store a UUID or idempotency key. Add the import `import { v4 as uuid } from "uuid";` and do:
```typescript
...
## Resiliency
:::info Bugs?
🚩 Explore the solution in `src/part3`, and run it with `npm run part3`.
...
:::tip Good news!
You don't need to do anything to benefit from these features.
:::

If you want to tune the default retry policies of the runtime, then have a look at the [Configuration docs](/deployment-operations/configuration).

### SDK utilities for retries

As we said, the runtime automatically does retries. The only thing that does not get retried are side effects.
If the result of a side effect is a failure, then this failure gets durable stored in the log.

But there are quite some scenarios where you would want a failed side effect to be retried.
For example, when the side effect tries to execute a payment.

To retry, the Typescript SDK offers some utilities. Let's activate retries for the
...
//highlight-end
```
Also add the following `import { RestateUtils } from "@restatedev/restate-sdk";`.
...
:::info Bugs?
🚩 Have a look at the fully implemented application in `src/part4`, and run it with `npm run part4`.
:::
