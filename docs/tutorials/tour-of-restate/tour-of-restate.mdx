...
You can call the `UserSessionService/Checkout` handler to proceed with the purchase, as follows:
...
You can also find `app.ts` there which contains the definition of the Restate server that will serve the RPC handlers.
...
Let's track the status of the tickets in the ticket service by storing it in the state. Do the tasks
below and check your implementation by running the service and calling the methods.
