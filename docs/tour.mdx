...
Try to adapt the `UserSession/addTicket` function to keep track of the cart items.
...
After you added the ticket to the cart array, the second highlighted line sets the state to the new value with `ctx.set(state_key: string, new_value: T)`.
...
Try to also adapt the `checkout` function in the user session service, to use the tickets:
```typescript
const checkout = async (ctx: restate.RpcContext, userId: string) => {
...
:::info
🚩 Explore the intermediate solution in `part2`, and run it with `npm run part2`.
:::
...
- `call(idempotencyKey: string, amount: number)` to execute the payment for a certain idempotency key and total amount.
...
:::info
🚩 Explore the intermediate solution in `part3`, and run it with `npm run part3`.
:::
...
You now call `paymentClient.failingCall` which fails 2 times before succeeding.
...
:::info 🚩 Have a look at the fully implemented app in `part4`, and run it with `npm run part4`.
:::
