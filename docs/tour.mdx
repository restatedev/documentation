...
2023-12-06T21:06:02.404764Z INFO restate_schema_impl::schemas_impl
  Registering endpoint
    ...
    restate.service_endpoint.address: http://localhost:9080/
...
So this means that if there is a call to `addTicket()`, then a call to `expireTicket()`, then `checkout()` will wait in line until `addTicket()` has finished, if and only if both were invoked with the same value of `key`.
This also counts if you omit the key parameter from the function signature, as we did for the `maybe()` and `withSomething()` functions above.
...
public BoolValue addTicket(RestateContext ctx, ReserveTicket request) throws TerminalException {
    ...
    //highlight-start
    var ticketClient = TicketServiceRestate.newClient(ctx);
    var success = ticketClient.reserve(Ticket.newBuilder().setTicketId(request.getTicketId()).build()).await();
...
The simplest way to make a call within Restate is to use the generated client, to call the other method. Use `.await()` to wait for the response to the call.
...
Once you have adapted the code, restart the service and try it out by calling the `UserSession.expireTicket` method as [explained earlier](#calling-services-from-outside-restate). In the service logs, you will see that the `addTicket` method doesn't wait for a response and completes before `reserve` does. The external call will get a response much quicker in this scenario.
...
const addTicket = async (
  ...
  // highlight-start
  await ctx.rpc(ticketServiceApi).reserve(ticketId);
  // highlight-end
  return true;
};
```
...
var client = UserSessionRestate.newClient(ctx);
var userSessionClnt = UserSessionRestate.newClient(ctx);
var req = ExpireTicketRequest.newBuilder().setTicketId(request.getTicketId()).setUserId(request.getUserId()).build();
userSessionClnt.delayed(Duration.ofMinutes(15)).expireTicket(req);
```
