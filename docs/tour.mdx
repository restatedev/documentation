...
When you are done with the tutorial, stop the runtime with `docker stop restate_dev`. This command also wipes the state (because you used the `--rm` flag).
...
You begin with request-response calls, in which one service calls another service and waits for the response.
...
:::tip
The suspension mechanism of Restate is especially beneficial if you run on serverless infrastructure, for example AWS Lambda.
...
### 📝 Try it out
In your example, when you add a seat to your shopping cart, it gets reserved for 15 minutes.
...
Applications often need to keep state. For example, your user session service
...
Remove the sleep that you used before.
