...
- Restate runtime Docker image: `restatedev/restate:VAR::RESTATE_VERSION`
...
- Restate runtime Docker image: `restate-dist:VAR::RESTATE_VERSION`
...
```shell
git clone --depth 1 --branch vVAR::TYPESCRIPT_SDK_VERSION git@github.com:restatedev/tour-of-restate-typescript.git
```
...
```shell
git clone --depth 1 --branch vVAR::JAVA_TOUR_VERSION https@github.com:restatedev/tour-of-restate-java.git
```
...
When you are finished with the tour, you can stop Restate with `docker stop restate_dev`. This will also delete any persisted state because we start the container with the `--rm` flag. You can restart Restate at a later time (without losing state) with `docker restart restate_dev`. You can view Restate logs using `docker logs restate_dev` or follow them live them by adding `--follow`.

When Restate first comes up, it doesn't know anything about our services. Let's tell it where they are running. You can trigger service discovery by calling the Restate admin API informing it about the endpoint where our service is listening.
...
Restate will send a discovery request to the service endpoint to find the services and methods run behind this endpoint. Restate keeps track of which services run where (including of the method signatures). Restate also keeps track of multiple revisions asyour APIs evolve, and routes requests appropriately.
...
2023-12-06T21:06:02.404764Z INFO restate_schema_impl::schemas_impl
  Registering endpoint
    restate.service_endpoint.id: aG9zdC5kb2NrZXIuaW50ZXJuYWw6OTA4MC8=
    restate.service_endpoint.address: http://host.docker.internal:9080/
...
```shell
curl -X POST http://localhost:8080/example.UserSession/AddTicket \
    -H 'content-type: application/json' \
...
```shell
curl -X POST http://localhost:8080/example.UserSession/Checkout \
    -H 'content-type: application/json' \
    -d '{"user_id": "123"}'
```
...
To make the call, you supply the method reference and request to `ctx.call`. Use `.await()` to wait for the response to the call.
...
```log
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.UserSession/AddTicket
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.UserSession/AddTicket
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=WAITING_START, id=K1qex52CPYkAYxA-vUucHa6j_MZxJ2b4A) to REPLAYING
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=REPLAYING, id=K1qex52CPYkAYxA-vUucHa6j_MZxJ2b4A) to PROCESSING
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=PROCESSING, id=K1qex52CPYkAYxA-vUucHa6j_MZxJ2b4A) to CLOSED
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:206 - Closing state machine
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.TicketService/Reserve
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.TicketService/Reserve
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=WAITING_START, id=IwF2v7ci3eUAYxA-vVYcWmtxF7ZiHRtJA) to REPLAYING
2023-12-06 23:13:57 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=REPLAYING, id=IwF2v7ci3eUAYxA-vVYcWmtxF7ZiHRtJA) to PROCESSING
```
...
:::caution
This isn't the best way to sleep in a Restate service!
The Restate SDK offers a way to sleep and immediately suspend, which we will cover a bit [later on](#suspendable-sleep) in the tour.
:::
...
TODO: Are we supposed to be seeing a suspension in the Java service too? I don't see it.

```log
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.UserSession/AddTicket
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.UserSession/AddTicket
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=WAITING_START, id=jRn_FAzvp8UAYxBCfRLeZi7nlbVewfICQ) to REPLAYING
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=REPLAYING, id=jRn_FAzvp8UAYxBCfRLeZi7nlbVewfICQ) to PROCESSING
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.TicketService/Reserve
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.TicketService/Reserve
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=WAITING_START, id=IwF2v7ci3eUAYxBCfTid9qSwFOBsES5tg) to REPLAYING
2023-12-06 23:30:20 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=REPLAYING, id=IwF2v7ci3eUAYxBCfTid9qSwFOBsES5tg) to PROCESSING
2023-12-06 23:30:55 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=PROCESSING, id=IwF2v7ci3eUAYxBCfTid9qSwFOBsES5tg) to CLOSED
2023-12-06 23:30:55 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:206 - Closing state machine
2023-12-06 23:30:55 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=PROCESSING, id=jRn_FAzvp8UAYxBCfRLeZi7nlbVewfICQ) to CLOSED
2023-12-06 23:30:55 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:206 - Closing state machine
```
...
You can also make one-way calls where you don't wait for the response.
...
Once you have adapted the code, try it out by calling the `UserSession/addTicket` function, as [explained earlier](#running-the-services-and-runtime).
...
All you need to do is use `ctx.oneWayCall`, as you do here in the `UserSession.addTicket` method:
...
Once you have adapted the code, try it out by calling the `UserSession.addTicket` method, as [explained earlier](#running-the-services-and-runtime).
In the service logs, you see that the `addTicket` function doesn't wait for a response and finishes earlier than the `reserve` function.

<details><summary>Show the logs</summary>

```log
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.UserSession/AddTicket
//highlight-next-line
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.UserSession/AddTicket
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=WAITING_START, id=K1qex52CPYkAYxA2Zwbc5mRfj-gVKileg) to REPLAYING
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=REPLAYING, id=K1qex52CPYkAYxA2Zwbc5mRfj-gVKileg) to PROCESSING
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.UserSession', state=PROCESSING, id=K1qex52CPYkAYxA2Zwbc5mRfj-gVKileg) to CLOSED
//highlight-next-line
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:206 - Closing state machine
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG RequestHttpServerHandler:157 - Handling request to example.TicketService/Reserve
//highlight-next-line
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG RestateGrpcServer:112 - Start processing call to example.TicketService/Reserve
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=WAITING_START, id=IwF2v7ci3eUAYxA2ZzlfgqxAKDFTcalTA) to REPLAYING
2023-12-06 22:37:32 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=REPLAYING, id=IwF2v7ci3eUAYxA2ZzlfgqxAKDFTcalTA) to PROCESSING
2023-12-06 22:38:07 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:665 - Transitioning InvocationStateMachine(serviceName='example.TicketService', state=PROCESSING, id=IwF2v7ci3eUAYxA2ZzlfgqxAKDFTcalTA) to CLOSED
2023-12-06 22:38:07 [vert.x-eventloop-thread-0] DEBUG InvocationStateMachine:206 - Closing state machine
```
...
Call the `expireTicket` function with:
```shell
curl -X POST http://localhost:8080/example.UserSession/ExpireTicket -H 'content-type: application/json' -d '{"user_id": "123"}'
```
