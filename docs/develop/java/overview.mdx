...
<CH.Section>
    ...
    - [The other handlers of the workflow are used to interact with the workflow: either query it, or signal it. They use the `WorkflowSharedContext` to interact with the SDK. These handlers can run concurrently with the `run` handler and can still be called after the `run` handler has finished.](focus://12:15)
    - [The `WorkflowContext` and `WorkflowSharedContext` give access to some additional features](focus://5,13): [workflow K/V state](/develop/java/state), and [Durable Promises](/develop/java/workflow_signals). Have a look at their respective docs to learn more.
