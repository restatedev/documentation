...
Serializers and deserializers are abstracted using the interface `dev.restate.sdk.common.Serde`. You can mix and match those implementations, independently from whether you're using Java or Kotlin.

We do suggest though to use [Jackson serialization](#jackson-serialization) for Java and [Kotlin serialization](#kotlin-serialization) for Kotlin.
