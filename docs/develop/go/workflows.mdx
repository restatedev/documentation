...
<Admonition type="note" title={"Workflow retention time"}>
    ...
    The retention time can be configured via the via the [UI](/develop/local_dev#restate-ui) or [Admin API](/references/admin-api/#tag/service/operation/modify_service) per Workflow definition by setting `workflow_completion_retention`.
</Admonition>
