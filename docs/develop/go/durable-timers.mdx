...
To schedule a handler to be called at a later time, have a look at the documentation on [delayed calls](/develop/go/service-communication#delayed-calls).
...
```go
CODE_LOAD::go/develop/durabletimers.go
```
