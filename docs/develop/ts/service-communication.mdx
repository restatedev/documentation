...
<Admonition type="note" title="Workflow retention time">
    ...
    This can be configured via the UI or [Admin API](/references/admin-api/#tag/service/operation/modify_service) per Workflow definition by setting `workflow_completion_retention`.
</Admonition>
