...
In order to get this additional guarantee, you can provide an [idempotency key](https://docs.restate.dev/develop/ts/clients/#invoke-a-handler-idempotently) on any request using `restate.rpc.opts`/`restate.rpc.sendOpts`:
