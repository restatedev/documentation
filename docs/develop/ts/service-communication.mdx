...
```ts call_workflow.ts
CODE_LOAD::ts/src/develop/service_communication.ts#<start_request_response_workflow>-<end_request_response_workflow>
```
...
<Admonition type="info" title="Workflow retention time">
    Once the `run` handler has finished, the other handlers can still be called up to the retention time of the workflow, by default 24 hours.
