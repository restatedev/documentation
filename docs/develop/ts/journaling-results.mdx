...
2. **[`RestatePromise` combinators](/develop/ts/journaling-results#restate-promise-combinators)**: Log the order in which RestatePromises are resolved/rejected, to ensure deterministic replay.
