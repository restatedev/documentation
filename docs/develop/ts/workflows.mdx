...
<Scrollycoding>
    ...
    You serve workflows in the same way as Services and Virtual Objects: by binding them to an [HTTP endpoint](/develop/ts/serving) or [AWS Lambda handler](/develop/ts/serving#creating-a-lambda-handler).
