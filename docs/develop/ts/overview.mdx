...
<CH.Section>
    ...
    - [The `WorkflowContext` and `WorkflowSharedContext` give access to some additional features](focus://6,13): [workflow K/V state](/develop/ts/state), and [Durable Promises](/develop/ts/workflow_signals). Have a look at their respective docs to learn more.
</CH.Section>
