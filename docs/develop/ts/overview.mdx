...
- [The first argument of the handler must be the `ObjectContext` parameter](focus://6).
...
- [If you want to have a handler that executes concurrently to the others and doesn't have write access to the K/V state, use the `ObjectSharedContext`.](focus://9)
