...
- Specify [the service name.](focus://4) The service can then be called at `<RESTATE_INGRESS_URL>/MyService/greet`.
- [List the handlers.](focus://5:8)
    [Each handler has a name (`greet`) and a function that implements the handler logic.](focus://6)
...
- The handler input parameters and return type are optional and can be of any type, as long as they can be [JSON serialized](/develop/ts/serialization).
