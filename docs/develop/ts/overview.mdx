...
<Admonition type="info" title="TS Handler API vs. gRPC API">
Restate currently supports two TypeScript APIs for implementing services: the Handler API and the gRPC API.
The gRPC API requires you to specify the services, methods and data types via a Protobuf definition, as explained in the service contract docs.
