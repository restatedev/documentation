...
<CH.Section>
    ...
    - [Every workflow implementation needs to have a handler called `run` that implements the workflow logic. This handler uses the `WorkflowContext` to interact with the SDK. The `run` handler executes exactly one time per workflow exeuction/object.](focus://6)
    - [The other handlers of the workflow are used to interact with the workflow: either query it, or signal it. They use the `WorkflowSharedContext` to interact with the SDK. These handlers can run concurrently with the `run` handler and can still be called after the `run` handler has finished.](focus://13)
    - [The `WorkflowContext` and `WorkflowSharedContext` give access to some additional features: workflow state, and shared promises. Have a look at their respective docs to learn more.](focus://6,13)
</CH.Section>
