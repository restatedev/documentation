...
On the side of the service that you are going to call, you export the API definition, as follows (for more details look at the [serving docs](/develop/sdk/serving)):

```ts
CODE_LOAD::ts/src/develop/service_communication.ts
```
...
```ts
CODE_LOAD::ts/src/develop/one_way_call.ts
```
...
On the side of the service that you are going to call, you export the API definition as shown in the [request-response calls section](/develop/sdk/service-communication#request-response-calls).
