...
To schedule a handler to be called at a later time, have a look at the documentation on [delayed calls](/develop/python/service-communication#delayed-calls).
...
```python
CODE_LOAD::python/src/develop/durable_timers.py
```
