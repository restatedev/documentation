...
- Each [workflow definition](/develop/python/overview#workflows) has a `run` handler that is annotated with `@service_name.main()` and implements the workflow logic.
