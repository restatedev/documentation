...
<CH.Scrollycoding className={"spotlight-medium"}>
    ...
    For example, use [`ctx.run`](/develop/v/journaling-results#journaled-actions) to log intermediate results in Restate and avoid re-execution on replay.
    ...
    You serve workflows in the same way as Services and Virtual Objects: by binding them to an [HTTP endpoint](/develop/python/serving#creating-an-http-endpoint).
