...
You can re-attach to any request by using the `ctx.attach_invocation` API, providing the invocation ID:
...
You can [cancel an invocation](https://docs.restate.dev/operate/invocation/#cancelling-invocations) by using the `ctx.cancel_invocation` API, providing the invocation ID:
