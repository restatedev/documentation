...
- The handler input argument (at most one) and return type are optional and can be of any primitive type, a `TypedDict`. Check out the [serialization docs](/develop/python/serialization) for Pydantic support, dataclasses, or custom serialization.
