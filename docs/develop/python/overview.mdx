...
- The handler input argument (at most one) and return type are optional and can be of any primitive type or a `TypedDict`.
  ...
  For any other types, you can use [Pydantic](/develop/python/serialization#pydantic) or a [custom serializer](/develop/python/serialization#custom-serialization).
