...
The response contains the [Invocation identifier](#invocation-identifier). You can use this identifier to cancel or kill the invocation as described in [the below paragraph](/services/invocation#cancel-an-invocation).
...
```shell
restate invocations cancel <INVOCATION_IDENTIFIER>
```
...
```shell
restate invocations cancel --force <INVOCATION_IDENTIFIER>
```
