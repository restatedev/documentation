...
```bash
curl <RESTATE_META_URL>/deployments -H 'content-type: application/json' -d '{"uri": "<SERVICE_ENDPOINT_URI>"}'
```
...
```shell
restate deployments register --force <SERVICE_ENDPOINT_URI>
```
...
```bash
curl <RESTATE_META_URL>/deployments -H 'content-type: application/json' -d '{"uri": "<SERVICE_ENDPOINT_URI>", "force": true}'
```
