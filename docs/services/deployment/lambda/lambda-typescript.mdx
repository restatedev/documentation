...
Replace `<RESTATE_META_URL>` and the Lambda function ARN in the data field.
...
Connect to the Restate (e.g. via an SSH session if it is running on EC2) runtime and execute the registration curl command:
