...
You can then use the invocation ID to [kill the invocation](/services/invocation#cancel-an-invocation).
...
To retrieve the state of a specific service, service key and state key, do:

```sql
select * from state where service = 'MyServiceName' and service_key = 'myKey' and key = 'myStateKey';
```
...
:::danger Editing state during ongoing invocations
...
```shell
restate state edit counter bob -w
```
