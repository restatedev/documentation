...
An event handlers can profit from the same features as other Restate handlers, including durable execution, [service communication](/services/features/service-communication) and [side effects](/services/features/side-effects).
...
@Override
public void handle(RestateContext ctx, Event event) {
