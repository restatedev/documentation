...
```bash
curl <RESTATE_META_URL>/subscriptions -H 'content-type: application/json' -d '{"source": "kafka://<CLUSTER_NAME>/<TOPIC_NAME>", "sink": "service://<SERVICE_NAME>/<METHOD_NAME>"}'
```
...
```bash
curl <RESTATE_META_URL>/subscriptions -H 'content-type: application/json' -d '{"source": "kafka://my-cluster/my-topic", "sink": "service://myRouter/eventHandler"}'
```
...
```bash
curl <RESTATE_META_URL>/subscriptions -H 'content-type: application/json' -d '{"source": "kafka://my-cluster/my-topic", "sink": "service://MyService/Handle"}'
```
...
```bash
curl <RESTATE_META_URL>/subscriptions -H 'content-type: application/json' -d '{"source": "kafka://<CLUSTER_NAME>/<TOPIC_NAME>", "sink": "service://<SERVICE_NAME>/<METHOD_NAME>", "options": {"auto.offset.reset": "earliest"}}'
```
...
```shell
curl <RESTATE_META_URL>/subscriptions
```
...
```shell
curl -X DELETE <RESTATE_META_URL>/subscriptions/<SUBSCRIPTION_IDENTIFIER>
```
