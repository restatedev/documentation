...
* The event handler's key input type must either be `string` or `bytes`.
* In case of `string` key type, the Kafka record key must contain a valid UTF-8 string, otherwise Restate won't be able to deliver the event to the service.
