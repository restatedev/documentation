...
```java
var uuid = ctx.sideEffect(TypeTag.STRING_UTF8, () -> UUID.randomUUID().toString());
```

You need to define the return value type of the side effect, so that the Restate SDK knows how to serialize and deserialize it. You can do this by supplying the `TypeTag` or class, for example `TypeTag.STRING_UTF8` or `boolean.class`.  
...
```java
ctx.sideEffect(boolean.class, () -> {
  var result = paymentClient.call(txId, amount);
  if(result){
    // highlight-next-line
...
```java
  private boolean payment() {
    ...
    if(result){
      //highlight-next-line
      throw Status.UNKNOWN.withDescription("Payment failed").asRuntimeException();
    } else {
      return result;
    }
  };
  ...
  public void myMethod(Request request, StreamObserver<Empty> responseObserver) {
    ...
    //highlight-start
    try {
      ctx.sideEffect(boolean.class, this::payment);
