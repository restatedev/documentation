...
Use Restate's generated gRPC clients to do the call.
...
var greetReq = GreetRequest.newBuilder().setName("Pete").build();
GreetResponse response = client.greet(greetReq).await();
```
...
```java
GreeterRestate.GreeterRestateClient client = GreeterRestate.newClient(ctx);
var greetReq = GreetingRequest.newBuilder().setName("Pete").build();
client.oneWay().greet(greetReq);
```
...
GreeterRestate.GreeterRestateClient client = GreeterRestate.newClient(ctx);
var greetReq = GreetRequest.newBuilder().setName("Pete").build();
client.delayed(Duration.ofSeconds(1)).greet(greetReq);
```
