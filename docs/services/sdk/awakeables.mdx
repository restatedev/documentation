...
You need to define the return value type of the awakeable, so that the Restate SDK knows how to deserialize it. You can do this by supplying the `TypeTag` or class, for example `TypeTag.STRING_UTF8` or `boolean.class`.  
...
```java
AwakeableHandle awakeableHandle = ctx.awakeableHandle(awakeableId);
awakeableHandle.resolve(TypeTag.STRING_UTF8, "hello");	
```
...
```java
AwakeableHandle awakeableHandle = ctx.awakeableHandle(awakeableId);
awakeableHandle.reject("my error reason");
```
