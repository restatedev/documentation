...
```shell
curl -X POST <RESTATE_INGRESS_URL>/dev.restate.Awakeables/Resolve -H 'content-type: application/json' -d '{"id": "<AWAKEABLE_ID>", "json_result": <JSON_VALUE_RESULT>}'
```

For example:

```shell
curl -X POST localhost:8080/dev.restate.Awakeables/Resolve -H 'content-type: application/json' -d '{"id": "T4pIkIJIGAsBiiGDV2dxK7PkkKnWyWHE", "json_result": {"hello": "world"}}'
```

To reject an awakeable:

```shell
curl -X POST <RESTATE_INGRESS_URL>/dev.restate.Awakeables/Reject -H 'content-type: application/json' -d '{"id": "<AWAKEABLE_ID>", "reason": "<FAILURE_REASON>"}'
```

For example:

```shell
curl -X POST localhost:8080/dev.restate.Awakeables/Reject -H 'content-type: application/json' -d '{"id": "T4pIkIJIGAsBiiGDV2dxK7PkkKnWyWHE", "reason": "Very bad error!"}'
```
...
Additionally, you can have a look at the documentation on [how to invoke a service](../invocation.md#invoking-restate-services).