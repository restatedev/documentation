...
You can configure a service as `private`, such that you can't invoke it over HTTP, through the [UI](/develop/local_dev#restate-ui) or the Admin API ([docs](/references/admin-api#tag/service/operation/modify_service)):
