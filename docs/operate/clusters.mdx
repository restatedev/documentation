...
- [Provisioning clusters](/deploy/server/cluster/deployment)
...
<Step stepLabel="0" title="Starting point: single node">
    ...
    Follow the [cluster deployment instructions](/deploy/server/cluster/deployment) and ensure that:
...
<details>
    <summary>**Node id misconfiguration puts log server in `data-loss` state**</summary>
