...
The invocation ID starts with `inv_`, and can be found in the [UI](/develop/local_dev#restate-ui), the logs and traces (`restate.invocation.id`), for example:
...
You can cancel the invocation with [its ID](/operate/invocation?interface=curl#invocation-identifier) via the [UI](/develop/local_dev#restate-ui), the CLI or [Admin API](/references/admin-api):
...
[UI](/develop/local_dev#restate-ui)
You can kill an invocation via the, the CLI, or the [Admin API](/references/admin-api):
