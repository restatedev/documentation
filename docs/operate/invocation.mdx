...
You can cancel the invocation with [its ID](/operate/invocation?interface=curl#invocation-identifier)) using the CLI:
...
You can cancel the invocation with [its ID](/operate/invocation?interface=curl#invocation-identifier)) using [the Admin API](/references/admin-api)):
