...
You can cancel the invocation with [its ID](/operate/invocation?interface=curl#invocation-identifier) via the CLI or [Admin API](/references/admin-api):
