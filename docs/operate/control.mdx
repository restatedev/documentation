...
Starting with version 1.2, Restate includes a command line utility to connect to and control running Restate servers called `restatectl`. This tool is specifically designed for system operators to manage the Restate servers, and is distinct from the application developer-focused `restate` CLI tool, which is used to manage service deployments and invocations.
...
The `restatectl` tool communicates with Restate using the server's advertised address specified in the [server configuration](/operate/configuration/server) - by default TCP port 5122. To get an overview of a running server or cluster, use the `status` command:
