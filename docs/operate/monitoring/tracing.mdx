...
<Admonition type="info" title={"Filtering traces"}>
    You can configure a span/event filter in a similar fashion to the [log filter](/operate/monitoring/restate_logging#log-filter) setting the `tracing-filter` configuration entry.
</Admonition>
