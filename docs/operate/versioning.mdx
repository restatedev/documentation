...
It is not possible to remove a single service directly, but it is possible to remove the deployment containing it.
To remove a deployment, the suggested procedure is the following:

1. Get the deployment ID that is serving the latest revision of the service.
...
4. [Make the service private](/operate/invocation#private-services) to avoid accepting new HTTP requests.
5. Check whether the service has pending requests, and wait until they are resolved.
