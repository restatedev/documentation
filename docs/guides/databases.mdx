...
- **Efficient**: Computation and access patterns to state align naturally, you cannot get into situations where 100s of functions try to access/update the same database entry and contend on locks or interfere with each other's transaction.
