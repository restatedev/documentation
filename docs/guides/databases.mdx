...
- Efficient: Computation and access patterns to state align naturally, you cannot get into situations where 100s of functions try to access/update the same database entry and contend on locks or interfere with each others’ transaction.
...
- Serverless: relevant state is pushed with function invocations, e.g., to lambda. It is locally available when the function executes, no access delays, synchronization, wait times, etc.
- No additional dependency - just works when you develop locally, or migrate the Restate app to cloud/prod
...
- Modifiable only from the Virtual Object, not from arbitrary services. Any modification needs to be sent as a request to the Virtual Object.
...
This is purely opional, the example code and comments document the behavior well.
