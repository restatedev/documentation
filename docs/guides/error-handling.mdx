...
If you do not want an error to be retried, you need to specifically label it as a terminal error (see below).
...
- Handler-level retry policies are not planned.
</Admonition>
...
When you cancel an invocation, it throws a terminal error in the handler processing the invocation the next time it awaits a Promise or Future of a Restate Context action (e.g. run block, RPC, sleep,...; `CombineablePromise` in TypeScript, `Awaitable` in Java).
...
Restate also has timeouts to bound the response time of a service invocation.

There are two types of timeouts in Restate:

### Inactivity timeout

When Restate does not receive a next journal entry from a running handler within the inactivity timeout, it will ask the handler to suspend.
...
You can set the inactivity timeout via the UI or the [Restate Server configuration](/operate/configuration/server).
...
Have a look at the [**sagas**](/guides/sagas) guide to learn how to revert your system back to a consistent state after a terminal error.
...
Note that this does not catch errors that happen before the handler gets invoked (e.g. service does not exist, decoding errors, ...). Handle these as follows:
...
You could handle the decoding errors by letting the called handler take raw input and doing the decoding and validation itself.
In this case, it would be included in the try-catch block which would do the dispatching.
To handle the other errors, we recommend having the upstream caller wait for the response and handle the propagation to the DLQ.
