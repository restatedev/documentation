...
<div className={"row"}>
    <div className={"col col--6 col-lg-6 col-md-12"}>
        ...
        <p className={"use-case-description padding-bottom--lg text--left"}>
            Restate serves as the resilience and durability layer for applications.
...
<FeatureList className={"padding-bottom--lg"} features={[
    {
        ...
        description: "The Restate SDKs provide programming constructs with built-in retries and recovery. Restate ensures code runs to completion: failures are automatically retried and previous progress is recovered.",
    },
    {
        title: 'Consistent application state',
        iconPath: '/img/use_cases/data.svg',
        description: "Restate persists code execution, including state updates. State remains consistent with execution progress. Restate enforces a single writer per key to prevent lost updates and concurrency issues.",
    },
    {
        title: 'Resilient communication and scheduling',
        iconPath: '/img/use_cases/call.svg',
        description: "Restate makes all communication resilient: request-response calls, async messages, webhooks, and scheduled tasks. No need for message queues, custom retry logic, deduplication, or separate schedulers.",
    },
]}/>
...
<Scrollycoding className="workflows-scroller">
    ...
    Restate transforms the programming building blocks you already know into resilient constructs.
    Work with objects, functions, and promises as if failures don't exist.
    ...
    CODE_LOAD::java/src/main/java/usecases/microservices/Idempotency.java
    ```
    ...
    ```
    ```kotlin !!tabs Kotlin
    CODE_LOAD::kotlin/src/main/kotlin/usecases/microservices/Idempotency.kt
    ```
    ...
    ```
    ```go !!tabs Go https://github.com/restatedev/examples/blob/main/go/patterns-use-cases/src/durablerpc/client/client.go
    CODE_LOAD::go/usecases/microservices/service/client/client.go
    ```

    ```py !!tabs Python https://github.com/restatedev/examples/blob/main/python/basics/app/0_durable_execution.py
    CODE_LOAD::python/src/use_cases/microservices/subscription_service.py?1
    ```
    ```py !!tabs Python https://github.com/restatedev/examples/blob/main/python/patterns-use-cases/src/durablerpc/client.py
    ...
    Execute calls to other systems and APIs and persist their results.
    On retries, actions aren't re-executed —- previous results are recovered.
    Chain interactions with multiple systems, and Restate ensures all interactions complete successfully.
    ...
    Restate sits like a reverse-proxy or message broker in front of your services, proxying calls and ensuring they're delivered and processed.
    Make any type of call resilient:
    ...
    ```ts !!tabs TypeScript https://github.com/restatedev/examples/blob/main/typescript/basics/src/0_durable_execution.ts
    CODE_LOAD::ts/src/use_cases/microservices/subscription_service.ts
    ```
    ```ts !!tabs TypeScript https://github.com/restatedev/examples/blob/main/typescript/patterns-use-cases/src/durablerpc/express_app.ts
    ...
    ```java !!tabs Java https://github.com/restatedev/examples/blob/main/java/basics/src/main/java/durable_execution/SubscriptionService.java
    CODE_LOAD::java/src/main/java/usecases/microservices/SubscriptionService.java
    ```
    ```java !!tabs Java https://github.com/restatedev/examples/blob/main/java/patterns-use-cases/src/main/java/my/example/durablerpc/MyClient.java
    ...
    ```kotlin !!tabs Kotlin https://github.com/restatedev/examples/blob/main/kotlin/basics/src/main/kotlin/durable_execution/SubscriptionService.kt
    CODE_LOAD::kotlin/src/main/kotlin/usecases/microservices/service/SubscriptionService.kt
    ```
    ```kotlin !!tabs Kotlin
    ...
    ```go !!tabs Go https://github.com/restatedev/examples/blob/main/go/basics/part0/durableexecution.go
    CODE_LOAD::go/usecases/microservices/service/subscriptionservice.go
    ```
    ```go !!tabs Go https://github.com/restatedev/examples/blob/main/go/patterns-use-cases/src/durablerpc/client/client.go
    CODE_LOAD::go/usecases/microservices/service/client/client.go?1
    ```

    ```py !!tabs Python https://github.com/restatedev/examples/blob/main/python/basics/app/0_durable_execution.py
    CODE_LOAD::python/src/use_cases/microservices/subscription_service.py
    ```
    ```py !!tabs Python https://github.com/restatedev/examples/blob/main/python/patterns-use-cases/src/durablerpc/client.py
    ...
    ```
    ```go !!tabs Go https://github.com/restatedev/examples/blob/main/go/patterns-use-cases/src/durablerpc/client/client.go
    CODE_LOAD::go/usecases/microservices/service/client/client.go?2
    ```
...
<SpotlightCard>
    <FeatureSpotlight
        title={"Regular functions and services, in your existing infrastructure"}
        description={<p>Your code runs just like before: as Java/NodeJS/... applications on FaaS, Kubernetes, servers, containers. <br/>
            -- on-premises or in the cloud. Restate is a single binary that is easy to deploy and operate. Restate meets you where you are.</p>}
...
<ExampleWidget features={[
    {
        ...
        description: "Issue idempotent payments to providers and process responses, whether immediate or via webhooks.",
        links: [
    ...
    {
        title: 'Sagas',
        description: "Keep multiple systems in sync. For example, reserve a flight and car, then handle payment. If payment fails, automatically roll back reservations.",
        links: [{url: "/guides/sagas", icon: "/img/arrow-right.svg"}]
    },
    {
        title: 'Human-in-the-loop workflows',
        description: "Food ordering example: handle payment, wait until desired delivery time, notify restaurant to start preparation, confirm with restaurant, and deliver.",
        links: [
    ...
    },
    {
        ...
        description: "Tracking the state of a machine in our factory with a Virtual Object. Define state transitions in your handlers and store current state in Restate.",
        links: [
    ...
    },
    {
        ...
        description: "Use Restate to solve common access problems, race conditions, and inconsistencies caused by retries, concurrent requests, or zombie processes.",
        links: [
