...
<CH.Scrollycoding className="workflows-scroller">
    ...
    Functions execute with Durable Execution: their progress is tracked and they can be retried from the exact point before the crash, a bit like taking micro-checkpoints throughout the function execution.

    ```ts user_updates.ts mark=4
...
<CH.Scrollycoding className={"workflows-scroller"}>
    ...
    ```ts profile_service.ts mark=5,10,12,16,17
    CODE_LOAD::ts/src/use_cases/events_state.ts
    ```
    ...
    ```ts profile_service.ts mark=4,9,15
    CODE_LOAD::ts/src/use_cases/events_state.ts
    ```
