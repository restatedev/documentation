...
<Tabs groupId="sdk" queryString>
    <TabItem value="ts" label="TypeScript">
        ...
        <CH.Spotlight className={"spotlight-long-code"}>
            ...
            You can **signal the workflow**, to send information to it, via [Durable Promises](/develop/ts/workflow_signals).
    ...
    <TabItem value="java" label="Java">
        <CH.Spotlight className={"spotlight-long-code"}>
            ...
            You can **signal the workflow**, to send information to it, via [Durable Promises](/develop/java/workflow_signals).
            For example, the payment provider signals the workflow that the payment was successful by calling the `paymentWebhook`.
