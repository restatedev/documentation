...
```shell
curl -X POST http://localhost:8080/myservice/hello -H 'content-type: application/json' -d '{"request": "Restate"}'
```
...
```shell
curl -X POST http://localhost:8080/greeter.Greeter/Greet -H 'content-type: application/json' -d '{"name": "Restate"}'
```
