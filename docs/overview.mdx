---
id: OVERVIEW
title: Overview
slug: /
description: "Documentation overview"
hide_table_of_contents: true
---

import NavigationCards from "../src/components/NavigationCards";

# Welcome to Restate!

Restate is the simplest way to write resilient workflows, event-driven applications, and async tasks.

### First time here?

<NavigationCards features={[
{
title: 'Quickstart',
category: 'first_time',
stepLabel: '1',
description: (
<>
<p className="margin--none">
<i>Get up and running.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=ts"
role="button"><img className="buttonIcon" src="/img/typescript.svg" width="26"
alt="TypeScript Quickstart"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=java"
role="button"><img className="buttonIcon" src="/img/java.svg" width="28"
alt="Java Quickstart"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=kotlin"
role="button"><img className="buttonIcon" src="/img/kotlin.svg" width="26"
alt="Kotlin Quickstart"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=go"
role="button"><img className="buttonIcon" src="/img/go.svg" width="40"
alt="Go Quickstart"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=python"
role="button"><img className="buttonIcon" src="/img/python.svg" width="26"
alt="Python Quickstart"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/quickstart?sdk=rust"
role="button"><img className="buttonIcon" src="/img/rust.svg" width="26"
alt="Rust Quickstart"></img></a></div>
</>
),
},
{
title: 'Tour of Restate',
category: 'first_time',
stepLabel: '2',
description: (
<>
<p className="margin--none">
<i>Get a feeling for the Restate SDKs.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/tour?sdk=ts"
role="button"><img className="buttonIcon" src="/img/typescript.svg" width="26"
alt="TypeScript Tour"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/tour?sdk=java"
role="button"><img className="buttonIcon" src="/img/java.svg" width="28"
alt="Java Tour"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/tour?sdk=go"
role="button"><img className="buttonIcon" src="/img/go.svg" width="40"
alt="Go Tour"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/get_started/tour?sdk=python"
role="button"><img className="buttonIcon" src="/img/python.svg" width="28"
alt="Python Tour"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
                               href="/get_started/tour?sdk=rust"
                               role="button"><img className="buttonIcon" src="/img/rust.svg" width="26"
                                                  alt="Rust Tour"></img></a></div>
</>
),
},
{
title: 'Concepts',
category: 'first_time',
stepLabel: '3',
description: (
<>
<p className="margin--none">
<i>Polish your understanding.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/concepts/durable_building_blocks"
role="button"><img className="buttonIcon" src="/img/arrow-right.svg" width="24"
alt="Concepts"></img></a></div>
</>
),
},
]}/>

### A regular here?

<NavigationCards features={[
{
title: 'Develop',
category: 'familiar',
stepLabel: '1',
description: (
<>
<p className="margin--none">
<i>Implement your handlers.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/category/typescript-sdk"
role="button"><img className="buttonIcon" src="/img/typescript.svg" width="26"
alt="TypeScript SDK Documentation"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/category/javakotlin-sdk"
role="button"><img className="buttonIcon" src="/img/java.svg" width="30"
alt="Java SDK Documentation"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="/category/javakotlin-sdk?sdk=kotlin"
role="button"><img className="buttonIcon" src="/img/kotlin.svg" width="26"
alt="Kotlin SDK Documentation"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/category/go-sdk"
role="button"><img className="buttonIcon" src="/img/go.svg" width="40"
alt="Go SDK Documentation"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/category/python-sdk"
role="button"><img className="buttonIcon" src="/img/python.svg" width="30"
alt="Python SDK Documentation"></img></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg firstTimeButton"
href="https://docs.rs/restate-sdk/latest/restate_sdk/"
role="button"><img className="buttonIcon" src="/img/rust.svg" width="26"
alt="Rust SDK Documentation"></img></a></div>
</>
),
},
{
title: 'Deploy',
category: 'familiar',
stepLabel: '2',
description: (
<>
<p className="margin--none">
<i>Deploy Restate and your handlers.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/deploy/kubernetes"
role="button"><img className="buttonIcon" src="/img/kubernetes-logo.svg" width="24"
alt="Kubernetes deployment"></img> <span className="buttonText"></span></a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton"
href="/category/aws--aws-lambda"
role="button"><img className="buttonIcon" src="/img/aws-logo.svg" width="28"
alt="AWS deployment"></img></a></div>
</>
),
},
{
title: 'Operate',
category: 'familiar',
stepLabel: '3',
description: (
<>
<p className="margin--none">
<i>Observe and upgrade your handlers.</i>
</p>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton text-grey"
href="/operate/introspection"
role="button">○ Inspect</a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton text-grey"
href="/operate/versioning"
role="button">○ Version</a></div>
<div id="overviewButtonDiv"><a className="overviewButton btn btn-primary btn-lg familiarButton text-grey"
href="/category/monitoring"
role="button">○ Monitor</a></div>
</>
),
},
]}/>
