...
<Step end={true} stepLabel="🎉" title="Congratulations, you now have working setup!"/>
...
<Scrollycoding>
    ...
    ```ts !
    CODE_LOAD::ts/src/get_started/file_workflow.ts?1
    ```
    ...
    ```ts !
    CODE_LOAD::ts/src/get_started/file_workflow.ts?2
    ```
    # !!steps
    ...
    ```ts !
    CODE_LOAD::ts/src/get_started/file_workflow.ts?3
    ```
...
<summary>Running the file workflow locally</summary>
    ...
    ```ts app.ts
    CODE_LOAD::ts/src/get_started/file_workflow.ts#full
    ```
    ...
    ```shell
    curl -X POST http://localhost:8080/FileWorkflow/file488/run \
    ...
    <Terminal>
        ...
        ```
        ```shell !output
        ❯ [2024-11-12 20:40:08.149 +01:00] inv_1lww6dToghwj3jShyf2z5j91EWKm6OnRgB
    ...
    💡   This invocation is bound to run on deployment 'dp_14DypPEJ9kZcklM7zwdBs2J'. To guarantee
    ...
    ―――――――――――――――――――――――
    [Ingress]
    └──(this)─> FileWorkflow/23/run
    ▸
    ├──── ☑️  #1 Send NotificationService/notify inv_1ai7U4M25kAK317XiCwqn4cbqOgb2TSjTz
    ├──── ⏸️  #2 Promise approval
    ├──── ☑️  #3 Sleep until 2024-11-12 20:40:18.159 +01:00
    ├──── ☑️  #6 Send NotificationService/remind inv_1c0ewogz3NG95fXBsEJkJrLQZkIjhxb6LL
    ├──── ⏸️  #7 Promise approval
    ├──── ⏰ #8 Sleep until 2024-11-12 20:40:28.172 +01:00 (8 seconds and 768 ms left)
    └────>> running
    ```
    </Terminal>
