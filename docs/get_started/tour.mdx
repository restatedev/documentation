...
<Admonition type="info" icon="💡" title="Concurrency guarantees">
...
This also means that time-consuming operations in a keyed service, for example sleeps, lock that key/service for the entire operation.
Make sure you don't introduce cyclical request-response calls in your application.
...
When an incoming request is sent to Restate, the runtime first persists it. It establishes a connection to the user session service, and sends the request over to begin a new invocation. All communications between the service and other Restate services goes over this connection.
