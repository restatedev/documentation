...
import {Step, SubtleStep} from "../../src/components/Stepper";
...
<SubtleStep stepLabel="1" title="Set up the tutorial">
<Tabs groupId="sdk" queryString className={"display-none"}>
<TabItem value="ts" label="TypeScript">
...
<SubtleStep stepLabel="2" title="Run the services">
...
<SubtleStep stepLabel="3" title="Launch Restate">
...
<SubtleStep stepLabel="4" title="Register the services with Restate">
...
You see in the logs that both services get suspended.
...
This is similar to sending a message that gets delayed by a specified duration.
...
In the previous sections, you implemented parts of the `TicketObject` and `CartObject`.
But until now, we did not yet mention how we will keep track of the tickets in the user's cart.
Or how we will make sure that several users can add tickets simulatenously without interfering with each other, and without selling a single ticket multiple times.

That's where **Virtual Objects** come into play.
Virtual Objects are a special type of service that allows you to group handlers together, share state between them, and control the number of ongoing invocations.
Each Virtual Object has a unique key.
Invocations are sequenced per Virtual Object, so at most one handler can run at a time for a given Virtual Object.

The issues with our current implementation, can easily be solved by using Virtual Objects.
We will convert the `TicketObject` and `CartObject` services to Virtual Objects.

The `TicketObject` will be keyed by the ticket ID, and we will store the current status of the ticket in the state (`AVAILABLE`/`RESERVED`).
Because it's a Virtual Object, we can be sure that only one invocation can run at a time for the same ticket ID.
So if user reserves a ticket, we can be sure that no concurrent other requests reserved the same ticket at the same time.

<Admonition type={"tip"} title={"Virtual Objects simplify many use cases"}>
With access to consistent K/V state and strong concurrency guarantees, implementing the `CartObject` service in a resilient and consistent way becomes straightforward.
</Admonition>

We also need to convert the `CartObject` service to a Virtual Object, keyed by the user ID.
This way, we can store the user's cart in the state.
With only one ongoing invocation, we are guarded against many concurrency issues when updating the cart.
...
<Tabs groupId="sdk" queryString className={"display-none"}>
    ...
    <TabItem value="java" label="Java">
        ...
        To convert the `TicketObject` and `CartObject` services to Virtual Objects, you need to:
        1. Replace the `@Service` annotation by `@VirtualObject`
        2. Replace the `Context` parameter by `ObjectContext` in the handler functions. The `ObjectContext` provides additional capabilities compared to the regular `Context` (e.g. K/V state access).
        3. You call a Virtual Object by specifying its key.
...
<Admonition type="caution">
    Since only a single invocation can run at a time for a given virtual object, you need to be careful with long-running operations, such as sleeps.
    ...
    This would block any other invocations for the same cart ID for 15 minutes.
    So the user wouldn't be able to add any other tickets, nor buy the tickets.
...
Restate offers a key-value store to persistently store application state for Virtual Objects.
...
Restate ensures that you get the cart belonging to the current Virtual Object (user).
...
You can watch the state of the `CartObject` service, via the CLI or via `psql` as follows:
...
Send a checkout request as [earlier](#request-response-http-requests), and notice that the state is now empty.
...
The SDK offers `ctx.run` to store the return value of any function in the journal.
...
Use the `auxiliary/PaymentClient` stub to execute the payment via `PaymentClient.get().call(idempotencyKey, amount)`.
...
- Invoking handlers: over HTTP and programmatically with the SDK
- Request-response invocations
- Sending messages
- Durable Execution: retries, partial progress recovery, and suspensions
- Durable timers: sleeping and scheduling async tasks
- Virtual Objects: concurrency guarantees and shared state
- K/V state: storing and inspecting
- Storing computation results in the journal
- Tracing 
