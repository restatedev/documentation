...
<div className="col col--8 scrollable-column">
...
<SubtleStep stepLabel="1" title="Run the services">
<Tabs groupId="sdk" queryString className={"display-none"}>
<TabItem value="ts" label="TypeScript">
...
In `src/app` you will find the skeletons of the various services to help you start implementing the app.
    ...
    CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part1/checkout_service.ts#<start_checkout>-<end_checkout>
    ```
...
```shell
curl localhost:8080/CartObject/Mary/addTicket -d '"seat2B"'
```
...
<Tabs groupId="sdk" queryString className={"display-none"}>
    <TabItem value="ts" label="TypeScript">
        ...
        CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part1/cart_object.ts#<start_add_ticket>-<end_add_ticket>
        ```
        <details className="grey-details">
        ...
        Send a request to `CartObject/addTicket` as we did [previously](#request-response-http-requests), and have a look at the service logs.
    ...
    <TabItem value="java" label="Java">
        ...
        Once you have added this to the code, restart the service, call the `CartObject/addTicket` method as we did [previously](#request-response-http-requests), and have a look at the service logs.
...
```typescript cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part1/cart_object.ts#<start_expire_ticket>-<end_expire_ticket>
```
...
<Admonition type="info" title="HTTP">
You can also do one-way calls via `curl`. Just add `/send` to the handler path:
```shell
curl localhost:8080/CartObject/Mary/addTicket/send -d '"seat2B"'
```
...
<details>
    ...
    <Tabs groupId="sdk" queryString className={"display-none"}>
        <TabItem value="ts" label="TypeScript">
            ...
            ```typescript cart_object.ts
            CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part1/cart_object.ts#<start_checkout>-<end_checkout>
            ```

            Call `CartObject/checkout` as you did [earlier](#request-response-http-requests) and have a look at the logs again to see what happened:
...
<Tabs groupId="sdk" queryString className={"display-none"}>
    ...
    <TabItem value="java" label="Java">
        ...
        Add the following code to line 4 of the snippet, to let the code throw an exception after the call:
...
```typescript cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part2/cart_object.ts#<start_add_ticket>-<end_add_ticket>
```
...
```ts cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/cart_object.ts#<start_add_ticket>-<end_add_ticket>
```
...
```typescript cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/cart_object.ts#<start_checkout>-<end_checkout>
```
...
```ts cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/cart_object.ts#<start_expire_ticket>-<end_expire_ticket>
```
...
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/ticket_object.ts#<start_reserve>-<end_reserve>
```
...
```ts ticket_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/ticket_object.ts#<start_unreserve>-<end_unreserve>
```
...
```ts ticket_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part3/ticket_object.ts#<start_mark_as_sold>-<end_mark_as_sold>
```
...
1. **Generate an idempotency token** (payment identifier) and store it in Restate.
Once the token is stored, it will be the same on retries.
2. **Trigger the payment** via an external payment provider.
Execute the payment via `PaymentClient.get().call(idempotencyKey, amount)`.
...
```typescript checkout_service.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part4/checkout_service.ts#<start_side_effects>-<end_side_effects>
```
...
You can print the idempotency key to the console and then throw an error to see how the idempotency key remains the same during retries:
...
Remove the throwing of the error again before you continue.
...
- If the payment call was successful, then:
    ...
    - Mark all tickets as sold by calling `TicketObject/markAsSold` for each ticket.
- If the payment call was unsuccessful, then use the EmailClient to notify the users of the payment failure.
...
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part4/checkout_service.ts#<start_checkout>-<end_checkout>
```
...
```typescript cart_object.ts
CODE_LOAD::https://raw.githubusercontent.com/restatedev/examples/tour_new_api/tutorials/tour-of-restate-typescript/src/part4/cart_object.ts#<start_checkout>-<end_checkout>
```
...
<SubtleStep stepLabel="3" title="Send requests">
...
<div className="col col--6 fixed-column">
    <iframe
        style={{
        ...
        src="https://stackblitz.com/github/restatedev/examples/tree/tour_new_api/tutorials/tour-of-restate-typescript?startScript=app-dev&ctl=1&embed=1&file=src%2Fapp%2Fcheckout_service.ts,src%2Fapp%2Fticket_object.ts,src%2Fapp%2Fcart_object.ts,src%2Fapp%2Fapp.ts&hideExplorer=1&theme=dark&view=editor"
    ></iframe>
</div>
</div>
</div>
