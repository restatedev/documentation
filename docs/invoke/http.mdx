...
**To invoke Workflows**, construct the path as follows:
...
<Admonition type={"tip"} title={"Latch onto running invocations and workflows"}>
    You can latch onto running invocations and workflows to wait for them to finish or to peek at their output.
...
<Admonition type="note" title={"Not supported for workflows"}>
    ...
    Workflows can only we scheduled with a delay from within another Restate handler ([TS](/develop/ts/service-communication#delayed-calls)/[Java/Kotlin](/develop/java/service-communication#delayed-calls)).
</Admonition>
...
If you use the `/send` endpoint to invoke a [workflow](/concepts/services#workflows) or handler, then the invocation or workflow runs in the background.

Restate allows you to latch onto the ongoing invocation or workflows.
...
<CH.Spotlight className="durable-promises">
    ...
    Via [Invocation ID](/operate/invocation#invocation-identifier)
    ...
    <CH.Code>
        ...
        ```shell attach.sh
        curl localhost:8080/restate/invocation/myVirtualObject/myKey/myHandler/myIdempotencyKey/attach
        ```

        ---

        ```shell output.sh
        curl localhost:8080/restate/invocation/myVirtualObject/myKey/myHandler/myIdempotencyKey/output
        ```
