import {CodeWithNotes} from "../../src/components/code/code/code-with-notes";import Admonition from '@theme/Admonition';
...
<details className={"grey-details"}>
    ...
    You can tune the retention time on a service-level by using the [Admin API](/references/admin-api#tag/service/operation/modify_service)):
    ```shell
...
Depending on the SDKs, the rich input/output JSON schemas are included as well. At the moment rich schemas are supported only in Python SDK and Java SDK.
