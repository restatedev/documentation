...
type SubscriptionRequest = {creditCard: string, userId: any, subscription: string};

function createRecurringPayment(creditCard: string, paymentId: string): string { return "" }

function createSubscription(userId: string, subscription: string) {
  return undefined;
}

async function tryPaymentFiveTimes(ctx: restate.ObjectContext, req: SubscriptionRequest, paymentId: string) {
  await ctx.run(
      "payment",
